...
Set `telemetry_endpoint` to the collector's base URL. For a Prometheus started with `--web.enable-otlp-receiver` that is its `/api/v1/otlp` path; the SDK appends `/v1/metrics`. Headers the collector needs, such as a Cloudflare Access service token, go in the `gcrunner-telemetry-headers` secret as `Name=value,Name=value`. Terraform creates the secret; add its version by hand:

```sh
printf 'CF-Access-Client-Id=%s,CF-Access-Client-Secret=%s' "$ID" "$SECRET" \
  | gcloud secrets versions add gcrunner-telemetry-headers --data-file=-
```
...
|---|---|---|
| `gcrunner_jobs_total` | `repository`, `workflow`, `status`, `conclusion` | One per `workflow_job` webhook for a gcrunner job. `status` is `queued`, `in_progress` or `completed`; `conclusion` is set on completed jobs. |
| `gcrunner_queue_duration_seconds` | `repository`, `workflow` | Histogram of GitHub queued to job started. |
| `gcrunner_job_duration_seconds` | `repository`, `workflow` | Histogram of job started to job completed. |
